...
*/
	...
	//// ダミーフレームの生成 (使いまわしができる様に、連結後のサイズ分確保しておく)
...
/***
	...
	if( NoErr == ret ) {
		...
		if( 0 != mycls_pIport[ch]->GetFrameWidth() % (mycls_pIport[ch]->GetFramePixelNBit()/my_nChannel) ) {
			LOG(em_ERR), "[%s] %s pitchサイズ不整合 (%d)", my_sThreadName, mycls_pIport[ch]->GetBase(0).GetDeviceIP(), mycls_pIport[ch]->GetFramePixelNBit());	
			LOM(em_ERR), "[%s] %s pitchサイズ不整合 (%d)", my_sThreadName, mycls_pIport[ch]->GetBase(0).GetDeviceIP(), mycls_pIport[ch]->GetFramePixelNBit());	
			return ApErr;
		}
		...
		if( my_nFrameW != mycls_pIport[ch]->GetFrameWidth() ||
			...
			LOG(em_ERR), "[%s] %s XMLサイズ不整合 (w=%d/%d,h=%d/%d)", my_sThreadName, mycls_pIport[ch]->GetBase(0).GetDeviceIP(), my_nFrameW, mycls_pIport[ch]->GetFrameWidth(), my_nFrameH, mycls_pIport[ch]->GetFrameHeight());	
			LOM(em_ERR), "[%s] %s XMLサイズ不整合 (w=%d/%d,h=%d/%d)", my_sThreadName, mycls_pIport[ch]->GetBase(0).GetDeviceIP(), my_nFrameW, mycls_pIport[ch]->GetFrameWidth(), my_nFrameH, mycls_pIport[ch]->GetFrameHeight());	
			return ApErr;
		} else {

			LOG(em_INF), "[%s] %s 接続完了(w=%d,h=%d)[%s]", my_sThreadName, mycls_pIport[ch]->GetBase(0).GetDeviceIP(), my_nFrameW, my_nFrameH, sXmlPath);	
			LOM(em_INF), "[%s] %s 接続完了(w=%d,h=%d)[%s]", my_sThreadName, mycls_pIport[ch]->GetBase(0).GetDeviceIP(), my_nFrameW, my_nFrameH, sXmlPath);	
		}
	} else {
...
//-----------------------------------------------------------------------------------------------
		...
		if( ret == EV_IPORT_IMG ) {
			...
			for(DWORD ii=0; ii<my_nCamsetNum; ii++) {
				...
				void*	p[MAX_IPORT_CAMNUM][2];				// ホントはMAX_IPORT_CAMANGLEが正しいが、chとjjの配列要素を使う箇所が多くなるので、可読性優先で MAX_IPORT_CAMNUM にしている
				DWORD	len[MAX_IPORT_CAMNUM][2];
				WORD	seq_number[MAX_IPORT_CAMNUM][2];						// iPort番号はshortなので注意
				DWORD	seqwk[2];
				DWORD	status[MAX_IPORT_CAMNUM][2];							// 一応絵は取れたけども、なんかおかしい場合だと思う
				...
				for(DWORD jj=0; jj<my_nAngleNum; jj++) {
					...
					if( NoErr != ret[ch] ) {
						camset_noerr = false;
						LOM(em_ERR), "->ret=%d, st=%d:%d, wait=%02d, lost=%d, resend=%d, Fno=%d, seq(自,iport)=%d:%d/%d:%d", ret[ch], status[ch][0], status[ch][1], nQueCnt, nLost, nResend, my_nFrameNum, my_nSeqNum[ch], my_nSeqNumSlave[ch], seq_number[ch][0], seq_number[ch][1]);
						LOG(em_ERR), "->ret=%d, st=%d:%d, wait=%02d, lost=%d, resend=%d, Fno=%d, seq(自,iport)=%d:%d/%d:%d", ret[ch], status[ch][0], status[ch][1], nQueCnt, nLost, nResend, my_nFrameNum, my_nSeqNum[ch], my_nSeqNumSlave[ch], seq_number[ch][0], seq_number[ch][1]);

					} else {
						LOM(em_MSG), "->ret=%d, st=%d:%d, wait=%02d, lost=%d, resend=%d, Fno=%d, seq(自,iport)=%d:%d/%d:%d", ret[ch], status[ch][0], status[ch][1], nQueCnt, nLost, nResend, my_nFrameNum, my_nSeqNum[ch], my_nSeqNumSlave[ch], seq_number[ch][0], seq_number[ch][1]);
					}
					...
					if( 0 != my_nSkipCnt[ch] ) {
						...
						LOG(em_MSG), "[%s] フレーム同期の為、スキップ (set=%d, ang=%d)(Fno=%d,iPort=%d:%d)",my_sThreadName, ii, jj, my_nFrameNum, seq_number[ch][0], seq_number[ch][1]); 
				...
				if(camset_noerr) {
					...
					for(DWORD jj=0; jj<my_nAngleNum; jj++) {
						...
						if(len[ch][0] != size0 || (status[ch][0] & 0x1f) ) {
							LOG(em_WAR), "[%s] ダミーフレームセット マスター (set=%d, ang=%d, no=%d) missing byte %d", my_sThreadName, ii, jj, seq_number[ch][0], size0-len[ch][0]);
							p[ch][0]  = my_pDummyBuf;
							len[ch][0]= size0;
						}
						if(len[ch][1] != size1 || (status[ch][1] & 0x1f) ) {
							LOG(em_WAR), "[%s] ダミーフレームセット スレーブ (set=%d, ang=%d, no=%d) missing byte %d", my_sThreadName, ii, jj, seq_number[ch][1], size0-len[ch][1]);
							p[ch][1]  = my_pDummyBuf;
							len[ch][1]= size1;
						}
					}
					...
					if( 0 == seq_number[ch][0] || 0 == seq_number[ch][1]  ) {
						if( 0 != my_nFrameNum && 0 == seq_number[ch][0] && 0 == seq_number[ch][1] ) {
							LOG(em_WAR), "[%s] iPortシーケンス番号取得失敗 (set=%d)(Fno=%d, 自=%d:%d, iPort=%d:%d)", my_sThreadName, ii, my_nFrameNum, my_nSeqNum[ch], my_nSeqNumSlave[ch], seq_number[ch][0], seq_number[ch][1]);
						}
					...
					for(DWORD jj=0; jj<my_nAngleNum; jj++) {
						...
						for(int kk=0; kk<2; kk++) {
							WORD& mseq = 0==kk ? my_nSeqNum[ch] : my_nSeqNumSlave[ch];			// エイリアス

							// 異常チェック (回復可能なエラー)
							//if( mseq != seq_number[ch][kk]+my_nChDiff[ch][kk] ) {
							if( (long)mseq != ((long)seq_number[ch][kk]+(long)my_nChDiff[ch][kk]) ) {

								if(0 == my_nFrameNum / my_nCamsetNum) {	// stopしたときのseq_numが引き継がれている場合があるので初回のみ特別扱い
									mseq = seq_number[ch][kk]+my_nChDiff[ch][kk];
								} else {
									LOG(em_WAR), "[%s] フレームズレ発生:シーケンス番号不一致 (set=%d, ang=%d, id=%d)(Fno=%d, 自:%d != iPort:%d + chdiff:%d)", my_sThreadName, ii, jj, kk, my_nFrameNum, mseq, seq_number[ch][kk], my_nChDiff[ch][kk]);
							...
							// シーケンス番号加算
							mseq ++;				// my_seq_numはiportと一致するようにインクリメントする (iPort番号がshortだから)

							// mValue で定義 (回復可能なエラー)
							if( IportBase::IsFrameOverrun(status[ch][kk]) ) {
								LOG(em_WAR), "[%s] フレームオーバーラン発生 (set=%d, ang=%d, id=%d, no=%d, st=%d)", my_sThreadName, ii, jj, kk, my_nFrameNum, status[ch][kk]);
							...
							if( IportBase::IsGrabberFIFOOverrun(status[ch][kk]) ) {
								LOG(em_WAR), "[%s] FIFOオーバーラン発生 (set=%d, ang=%d, id=%d, no=%d, st=%d)", my_sThreadName, ii, jj, kk, my_nFrameNum, status[ch][kk]);
							...
							if( IportBase::IsImageDropped(status[ch][kk]) ) {
								LOG(em_WAR), "[%s] IMAGE DROP発生 (set=%d, ang=%d, id=%d, no=%d, st=%d)", my_sThreadName, ii, jj, kk, my_nFrameNum, status[ch][kk]);
							...
							if( IportBase::IsPartialLineMissing(status[ch][kk]) ) {
								LOG(em_WAR), "[%s] PARTIAL LINE MISSING発生 (set=%d, ang=%d, id=%d, no=%d, st=%d)", my_sThreadName, ii, jj, kk, my_nFrameNum, status[ch][kk]);
							...
							if( IportBase::IsFullLineMissing(status[ch][kk]) ) {
								LOG(em_WAR), "[%s] FULL LINE MISSING発生 (set=%d, ang=%d, id=%d, no=%d, st=%d)", my_sThreadName, ii, jj, kk, my_nFrameNum, status[ch][kk]);
					...
					for(DWORD jj=0; jj<my_nAngleNum; jj++) {
						...
						if( ret[ch] == NoErr ) {
							...
							for(int kk=0; kk<2; kk++) {
								...
								// 異常チェック (回復可能なエラー)
								if( mseq != seq_number[ch][kk]+my_nChDiff[ch][kk] ) {
									LOG(em_WAR), "[%s] フレームズレ発生:シーケンス番号不一致 (set=%d, ang=%d, id=%d)(Fno=%d, 自=%d, iPort=%d + chdiff=%d)", my_sThreadName, 
...
void IportManager2p::OnFrameGrabbed(void** pImgs, long nX, long nY, DWORD nFrameNum, DWORD nCamset, WORD* nSeqNum, __int64 nFastTime)
{
	...
	for(DWORD ii=0; ii<my_nAngleNum; ii++) {
		...
		if( ! my_bImgTurn_LR ) {
			...
			//・右半分	SwdetectMMX::hflip_righthalf
			//	左画像
			//	右画像
			//	結合後の画像領域
			//	結合後の幅ピッチ
...
//// テスト
//		memcpy( &p->pImgs[ii*my_nImgSize], pImgs[2*ii], nX * nY );				// 左半分
//		memcpy( &p->pImgs[ii*my_nImgSize + (nX * nY)], pImgs[2*ii+1], nX * nY );	// 右半分のひっくり返してないやつ
...
//------------------------------------------
// 1カメラ内でiPortシーケンスNoズレ補正用 
// DWORD set カメラセット (0オリジン)
// DWORD ang カメラ角度 (0オリジン)
// int nSkipCnt (0:無し 1:マスターを1回スキップ -1:スレーブを1回スキップ)
//------------------------------------------
