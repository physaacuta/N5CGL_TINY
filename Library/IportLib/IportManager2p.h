...
//#ifdef IPORTLIB_IMG_COPY_MMX
	...
	//#include "..\..\EPC\lib\mmx\TapReconst.h"
//#endif
#include "..\KizuMMX\TapReconst.h"											// MMXライブラリ
#include "..\KizuMMX\MemCopy.h"												// MMXライブラリ



// ローカル
#include "IportBase2p.h"													// iPort基底クラス
...
namespace IportLib
{
	...
	// カメラ取込本体
	//===================================================================
	class IportManager2p : public IportManager
	{
	...
	//// 公開関数
	public:
		IportManager2p(CString sMyManagerName="iPortMgr2p", CString sMyIport="iPortBase");
		virtual ~IportManager2p(void);
		...
		virtual IportLib::ERRCODE	Connect(DWORD set, DWORD ang, CString sXmlPath, 
										...
										int nGrabQueSize=IportBase::GRAB_DEF_QSIZE) { _ASSERT(false); return NoErr; }	// iPortに接続 (1p) 設定:XML, Master:XML
		virtual IportLib::ERRCODE	Connect(DWORD set, DWORD ang, CString sXmlPath, 
										const char* mac_addr, const char* ip_addr, char const* mc_addr, 
										bool bSaveIpAddr=true, bool bIsSlave=false, long nAdapterIndex=0, 
										int nGrabQueSize=IportBase::GRAB_DEF_QSIZE) { _ASSERT(false); return NoErr; };	// iPortに接続 (1p) 設定:XML, Master:IP直
		...
		virtual IportLib::ERRCODE	Connect(DWORD set, DWORD ang, CString sXmlPath, long nAdapterIndex1,
										...
										bool bSaveIpAddr=true, bool bIsSlave=false, int nGrabQueSize=IportBase::GRAB_DEF_QSIZE);	// iPortに接続(2p)
		virtual IportLib::ERRCODE	Connect(DWORD set, DWORD ang, CString sXmlPath,
									    const char* mac_addr1, const char* ip_addr1, long nAdapterIndex1, char const* mc_addr1,
										const char* mac_addr2, const char* ip_addr2, long nAdapterIndex2, char const* mc_addr2,
										bool bSaveIpAddr=true, bool bIsSlave=false, int nGrabQueSize=IportBase::GRAB_DEF_QSIZE);	// iPortに接続(2p) 設定:XML, Master:IP直, Slave:IP直
		...
		virtual void		SetSyncPort(DWORD set, DWORD ang, int nSkipCnt);	// 1カメラ内でiPortシーケンスNoズレ補正用 (0:無し 1:マスターを1回スキップ -1:スレーブを1回スキップ)
	...
	public:
		...
		virtual inline IportBase&		GetClsIpot(DWORD set, DWORD ang, bool isPrimary=true)	    { return isPrimary ? GetBase(set, ang).GetBase(0) : GetBase(set, ang).GetBase(1); }
		...
		virtual inline bool IsPT2000() const		{return true;}
		...
		virtual DWORD		GetChDiff	  (DWORD set, DWORD ang) const { DWORD ch=GetCamIndex(set,ang); return  __max(my_nChDiff[ch][0], my_nChDiff[ch][1]);}
		virtual DWORD		GetChDiffMax()		const { return my_nChDiffMax;}


	//// 追加関係
	public:
		inline IportBase2p&		  GetBase(DWORD set, DWORD ang)		  { return (IportBase2p&)*mycls_pIport[GetCamIndex(set, ang)];}
		inline IportBase2p const& GetBase(DWORD set, DWORD ang) const { return (IportBase2p&)*mycls_pIport[GetCamIndex(set, ang)];}
	...
	protected:
		// クラス
		IportBase2p*					mycls_pIport[MAX_IPORT_CAMNUM];		// iPortクラス 

		// 基本情報
		WORD							my_nSeqNumSlave[MAX_IPORT_CAMNUM];	// 自分で持っているシーケンス番号 (スレーブ側) (iPort番号と一致するようにしている) (start時のシーケンス番号で初期化する)
		short							my_nChDiff[MAX_IPORT_CAMNUM][2];	// 自分で持っているシーケンス番号に対する各iportシーケンス番号との差
		short							my_nChDiffMax;
	};
};
