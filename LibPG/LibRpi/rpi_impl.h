...
//------------------------------------------
//基本クラス
//　読み書き用のバッファ＝（メモリポインタとサイズ）をセットで取り扱う
//------------------------------------------
struct MemoryData
{
	BYTE* pBuffer;
	...
	MemoryData(BYTE* abuffer = NULL, int acapacity = 0)
...
//------------------------------------------
struct MemoryDataVMAlloc  : MemoryData
{
public:
	static int PageAlignOf( int sz){ return (sz+4095) & 0x7ffff000; }

	MemoryDataVMAlloc(int buszFrame)
		:MemoryData( NULL,  buszFrame)
	{
		szCapacity =  buszFrame;
