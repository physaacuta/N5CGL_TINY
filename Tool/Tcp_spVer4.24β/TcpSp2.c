...
#define     ENQ_RSND_TIMER_ID  EB_TMRID_ENQSD    // ＥＮＱ再送タイマー
#define     AK0_RCV_TIMER_ID   EB_TMRID_AK0RV    // ＡＣＫ０受信待ちタイマー
#define     AK1_RCV_TIMER_ID   EB_TMRID_AK1RV    // ＡＣＫ１受信待ちタイマー
#define     TXT_RCV_TIMER_ID   EB_TMRID_TXTRV    // テキスト受信待ちタイマー
#define     EOT_RCV_TIMER_ID   EB_TMRID_EOTRV    // ＥＯＴ受信待ちタイマー
...
unsigned char   g_pRecvBuf[3][COM_BUF_MAX];		// 受信ﾃﾞｰﾀ格納用
...
struct sockaddr_in  UdpMyAddr[3];               // 接続元アドレス (UDP)
struct sockaddr_in  UdpDsAddr[3];               // 接続先アドレス (UDP)
struct sockaddr_in  recvAddr;                   // 送信元アドレス (UDP)
...
//				EnableWindow( GetDlgItem(hWnd, IDC_EDIT_MSG3 ), FALSE );
		...
		case IDCANCEL_BTN:				// 終了
            if ( TRUE == isConnectPort(PORT1) || TRUE == isConnectPort(PORT2) ) {
                OkNgMsgBox( hWnd, "回線接続中です。よろしいですか？", "TcpSpを終了します。");
	            if(1 != g_okng) {
                    break;
                }
            }
	...
	case WM_USER_ASYNC_SELECT+PORT3:		// ソケットメッセージ
...
BOOL ConnectSocket( HWND hWnd, int port )
{
    ...
    if ( PORT3 == port ) {
    	...
    	if( bind( g_sSock[tblno], (SOCKADDR *)&UdpMyAddr[tblno], sizeof(SOCKADDR_IN )) ) {
    		...
    		MyMessage( hWnd, "UDP (bind)　Recvポートのオープンに失敗 (Link Port)");
    		return FALSE;
    	}
        MyMessage( hWnd, "Link Port MyPort=%d オープン完了", usport );
...
//    	UdpDsAddr[tblno].sin_port        = htons(usport);            /* ポート番号           */
		...
		if ( WSAAsyncSelect( g_sSock[tblno], hWnd, WM_USER_ASYNC_SELECT+port, FD_READ | FD_WRITE | FD_CLOSE )
	        ...
	        closesocket( g_sSock[tblno] );
   	        g_sSock[tblno] = INVALID_SOCKET;
			MyMessage( hWnd, "PORT %d  非同期selectの実行エラー (Link Port)", port );
    		return FALSE;
        }
        MyMessage( hWnd, "Link Port SendPort=%d オープン完了", udp_send_port );
        return TRUE;
    }
    	...
    	if(isConnectType(port)) {
            ...
            if(PCTERM == tcpCtrl.comType || 0 != tcp_port1) {      // PCTERM ? ﾎﾟｰﾄ設定有り？
    		    ...
    		    if ( bind( g_sSock[tblno], (LPSOCKADDR)&addr, sizeof(addr)) != 0 ) {
    		        closesocket( g_sSock[tblno] );
	    	        g_sSock[tblno] = INVALID_SOCKET;
        			MyMessage( hWnd, "PORT %d  バインドエラー(bind error).　-%d-", port, __LINE__ );
        			return FALSE;
    		    }
            }
...
void SendMsg( HWND hWnd, int port )
{
    ...
    int     tblno;
    int     offset;
    int     iebctr_idx;                         /* Ｅｎｅｔ－Ｂ制御文字配列ＩＮＤＥＸ */
    char    c_dle[1];
    char    c_stx[1];
    char    c_etx[1];
    char    c_enq[1];
...
//            MakeFullName(hWnd, tcpCtrl.traceName, fname);
    ...
    if(isTcpType()) {
        ...
        if ( UdpDsAddr[tblno].sin_port == UdpMyAddr[tblno].sin_port ) {
            nsent = sendto( g_sSock[tblno], g_pSendBuf, nstart, 0,
                (struct sockaddr *)&recvAddr,           // 送信先ソケットのアドレスへのポインタ
    ...
    if ( nsent == SOCKET_ERROR ) {
        ...
        if ( tcpCtrl.comType == ENET_B || tcpCtrl.comType == ENET_BX) {
            switch ( iebctr_idx ) {
            case ENBCTL_IXENQ:          /* ＥＮＱ送信   */
                                        /* ＡＣＫ０待ちタイマーＳＥＴ */
                Ack0TimerSet ( hWnd );
                g_eb_sndst = EB_ENQSND;
                break;

            case ENBCTL_IXAK0:         /* ＡＣＫ０送信 */
                                       /* テキスト待ちタイマーＳＥＴ */
                TextTimerSet ( hWnd );
                g_eb_sndst = EB_AK0SND;
                break;

            case ENBCTL_IXAK1:         /* ＡＣＫ１送信 */
                                       /* ＥＯＴ待ちタイマーＳＥＴ   */
                EotTimerSet ( hWnd );
                g_eb_sndst = EB_AK1SND;
                break;

            case ENBCTL_IXEOT:         /* ＥＯＴ送信   */
                                       /* Ｅｎｅｔ－Ｂ伝送終了処理 */
                EnetB_DBend ( hWnd );
                break;
...
//    struct sockaddr_in  recvAddr;                       // 送信元アドレス (UDP)
    ...
    if(PORT3 == port) {
        ...
        nrecv = recvfrom( g_sSock[tblno],                 // ソケット（Soket Descriptor）
                        ...
                        (struct sockaddr *)&recvAddr,    // 送信側のアドレスを持つバッファへのポインタ
                        &SockaddrSize );                 // 送信側のアドレスを持つバッファのサイズへのポインタ
        if ( nrecv == SOCKET_ERROR ) {
            nerror = WSAGetLastError( );
            if ( nerror != WSAEWOULDBLOCK ) {
                MyMessage( hWnd, "Link Port 受信エラー　Error No. %d",nerror );
            }
        }
        MyMessage(hWnd, " " );
...
//                  PostMessage( hWnd, WM_USER_ASYNC_SELECT+port, g_sSock[tblno], WSAMAKESELECTREPLY( FD_WRITE, 0 ));
    ...
    if ( ENET_B == tcpCtrl.comType ) {
        ...
        switch ( iebctr_idx ) {
        case ENBCTL_IXENQ:    /* ＥＮＱ受信   */
            if ( EB_NOTSND != g_eb_sndst && EB_ENQSND != g_eb_sndst ) {
                MyMessage ( hWnd, "ポート%d：予期せぬＥＮＱ受信。 LINE:%d 送信中定数:%d ", port, __LINE__, g_eb_sndst );
                return;
            }
                              /* ＥＮＱ衝突対応 */
                              /* ＡＣＫ０受信待ちタイマーキャンセル */
            ...
            MyMessage(hWnd,"LINE:%d ポート%dへＡＣＫ０送信", __LINE__, port );
            SendMsg ( hWnd, port );

            break;

        case ENBCTL_IXAK0:    /* ＡＣＫ０受信 */
            if ( EB_ENQSND != g_eb_sndst ) {
                MyMessage ( hWnd, "ポート%d：予期せぬＡＣＫ０受信。 LINE:%d 送信中定数:%d ", port, __LINE__, g_eb_sndst );
                return;
            }
                              /* ＡＣＫ０受信待ちタイマーキャンセル */
            Ack0TimerCancel ( hWnd );
            ...
            if ( eb_sndbuf.in_point != eb_sndbuf.out_point ) {
                ...
                MyMessage(hWnd,"LINE:%d ポート%dへテキスト送信", __LINE__, port );
                SendMsg ( hWnd, port );

            } else {
                MyMessage ( hWnd, "ポート３：送信テキスト無し。 LINE:%d INPOINT:%d OUTPOINT:%d ",
        ...
        case ENBCTL_IXAK1:    /* ＡＣＫ１受信 */
            if ( EB_TXTSND != g_eb_sndst ) {
                MyMessage ( hWnd, "ポート%d：予期せぬＡＣＫ１受信。 LINE:%d 送信中定数:%d ", port, __LINE__, g_eb_sndst );
                return;
            }
                              /* ＡＣＫ１待ちタイマーキャンセル */
            ...
            MyMessage(hWnd,"LINE:%d ポート%dへＥＯＴ送信", __LINE__, port );
            SendMsg ( hWnd, port );

            break;

        case ENBCTL_IXEOT:    /* ＥＯＴ受信   */
            if ( EB_AK1SND != g_eb_sndst ) {
                MyMessage ( hWnd, "ポート%d：予期せぬＥＯＴ受信。 LINE:%d 送信中定数:%d ", port, __LINE__, g_eb_sndst );
                return;
            }
                              /* ＥＯＴ待ちタイマーキャンセル */
            EotTimerCancel ( hWnd );
        ...
        case ENBCTL_IXNAK:    /* ＮＡＫ受信   */
            ...
            if ( EB_ENQSND == g_eb_sndst ) {
                ...
                if ( tcpCtrl.nm_txt_anak > g_eb_cnttxt ) {
                    ...
                    memcpy( g_pSendDmy, eb_sndbuf.snd_buff[eb_sndbuf.out_point], eb_sndbuf.snd_size[eb_sndbuf.out_point] );
                    g_nTotalToSend = eb_sndbuf.snd_size[eb_sndbuf.out_point];
                    MyMessage(hWnd,"LINE:%d ポート%dへテキスト再送", __LINE__, port );
                    SendMsg ( hWnd, port );
                              /* リトライ回数更新 */
                    g_eb_cnttxt = g_eb_cnttxt + 1;

                } else {
                    MyMessage ( hWnd, "テキスト再送リトライ回数オーバー。 LINE:%d ", __LINE__ );
                              /* 送信テキストバッファ最古レコード削除 */
                    Ebsdbuf_oldrecdel ( hWnd );
                              /* リトライ回数初期化 */
                    g_eb_cnttxt = 0;
                ...
                MyMessage ( hWnd, "ポート%d：予期せぬＮＡＫ受信。 LINE:%d 送信中定数:%d ", port, __LINE__, g_eb_sndst );
                return;
            }
        ...
        default:              /* テキスト */
            if ( EB_AK0SND != g_eb_sndst ) {
                MyMessage ( hWnd, "ポート%d：予期せぬテキスト受信。 LINE:%d 送信中定数:%d ", port, __LINE__, g_eb_sndst );
                return;
            }
                              /* テキスト待ちタイマーキャンセル */
            ...
            MyMessage(hWnd,"LINE:%d ポート%dへＡＣＫ１送信", __LINE__, port );
            SendMsg ( hWnd, port );
                              /* 受信中伝文情報ＳＥＴ */
            g_eb_rcvsiz3 = nrecv;
...
//	char		cpszmsg[100];
    ...
    /***************************************************************************************/
    /* 2009 1/16 UMEMOTO アクセプト中に同一ポートに対して複数回の接続時要求がされた場合に  */
    /* 最終的に接続要求されたポートを優先させる。                                          */
    /* （同一ポートに再接続されたら、一旦切断して再接続させるため）                        */
    /***************************************************************************************/
    if ( g_sSockClient[tblno] != INVALID_SOCKET ) {
        MyMessage(hWnd, "PORT %d  接続中ポートに接続要求がありました(accept).", port );
        MyMessage(hWnd, "PORT %d  接続中ポートをクローズします", port );
		CloseSocket( hWnd, port );
	}
            ...
            if(0 == memcmp(&SinaDef[SinaPoint].fname[len-3], "bin", 3)) {
                ...
                if(MakeDefData(hWnd, SinaDef[SinaPoint].fname, NULL)) {
                    ...
                    if(1 == g_SinaSozOpen) {
               		...
               		MyMessage(hWnd, "ｼﾅﾘｵ定義ﾌｧｲﾙ変換異常 (%d) [%s]",
            ...
            if(0 == memcmp(&NextRecvSinaDef[i].fname[len-3], "bin", 3)) {
                ...
                if(MakeDefData(hWnd, NextRecvSinaDef[i].fname, RecvSinaData[i])) {
                    SendMsg(hWnd, NextRecvSinaDef[i].port);
               		...
               		MyMessage(hWnd, "受信ｼﾅﾘｵ定義ﾌｧｲﾙ変換異常 (%d) [%s]",
...
#ifdef COMMENT     // #define COMMENT がコメント(// #define COMMENT )の時、期限無しとなる。
    hani = 0;
...
//                     kikan[0], kikan[1], kikan[2]);
        sprintf(msg, "試行期間は残り %d 日と %d 時間 %d 分です。\n",
                     kikan[0], kikan[1], kikan[2]);
...
//                         "もう使えません！！  ψ(｀∇´)ψ ｹｹｹｯ", MB_ICONSTOP | MB_OK);
        MessageBox(hWnd, "期限が切れました。", "使用できません", MB_ICONSTOP | MB_OK);
        //exit(0);
		g_flag = 1;
    } else {
//        sprintf(msg, "試行期間は残り %d 日と %d 時間 %d 分です。　(＾▽＾ｹｹｹ\n\n"
