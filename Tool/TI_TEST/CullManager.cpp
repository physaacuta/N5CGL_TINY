...
void CullManager::Exec(DELI* pDeli)
{
	...
	LOG(em_INF), "[%s] 全長送信開始 <%d> [%d]  QueFree:%d", my_sThreadName, pDeli->nCamId, pDeli->nFrameNo, mque_pSockRpi->GetCountFree());
	...
	if (NULL == pRpiH) {
		...
		for(int ii=0; ii<MAX_IMAGE_CNT; ii++) {
			...
			if( 0 == pDeli->nFrameSkip ) {
				...
				KizuMMX::mem_copy_64xmm(&pDeli->pImg[nImgFromOffset], pRpiH->nWidth, &pImg[nImgSetSize], pRpiH->nWidth, pRpiH->nWidth, pRpiH->nHeight);
			} else {
				// ダミー画像
...
kb.DataToBMPfile_Gray(pRpiH->nWidth, pRpiH->nHeight, &pImg[ pRpiH->nImgOffset[ii] ], s);
		...
		LOG(em_MSG), "[%s] 全長送信依頼 <cam=%d> [%d] %dByte", my_sThreadName,	pRpiH->nCamId, pRpiH->nFrameNo, nImgSetSize );
		LOGCULL(em_MSG), "[%s] 全長送信依頼 <cam=%d> [%d] %dByte", my_sThreadName,	pRpiH->nCamId, pRpiH->nFrameNo, nImgSetSize );
	...
	LOG(em_INF), "[%s] 全長送信終了 <%d> [%d] QueFree:%d", my_sThreadName, pDeli->nCamId, pDeli->nFrameNo, mque_pSockRpi->GetCountFree());
	//// 時間
