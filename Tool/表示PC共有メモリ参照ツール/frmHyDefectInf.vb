...
Public Class frmHyDefectInf
	...
	Private Sub frmHyDefectInf_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
		...
		'InitDgv(dgvDefect1, 61)
		InitDgv(dgvDefect1, 63)
		'// 2023.09.14 画像処理装置 H・Sソフト改造 <<<<<--------------------
		...
		str &= String.Format("TBL_HY_DEFECTINF Size = {0}", Marshal.SizeOf(GetType(TBL_HY_DEFECTINF))) & vbCrLf
    ...
    Private Sub m_timer2_tick(ByVal sender As Object, ByVal e As EventArgs) Handles m_timer2.Tick
		...
		Dim tmp As DEFECTINF_HEADER = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, 0, GetType(DEFECTINF_HEADER), nRetc), DEFECTINF_HEADER)
		If CBool(nRetc) Then
			' 共有メモリにアクセス可能
			DspShmemStatus(True)
    ...
    Private Function ReadFromShmem(ByVal rec As Integer, ByVal pnt As Integer) As Boolean
        ...
        Try
            ...
            'm_typDefectInfHeader = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nTopByte, GetType(DEFECTINF_HEADER), nRetc), DEFECTINF_HEADER)

            nReadAddr = SIZE_DSP_MAP_DEFECTINF * (rec - 0)
            m_typDefectInfHeader = CType(ksMemReadMap(TBL_HY_DEFECTINF_NAME, nReadAddr, tMod_MemSet.SizeOf(m_typDefectInfHeader), GetType(DEFECTINF_HEADER), nRetc), DEFECTINF_HEADER)
            ...
            'm_typDefectInfPnt = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nTopByte, GetType(DEFECTINF_PNT), nRetc), DEFECTINF_PNT)
            nReadAddr = SIZE_DSP_MAP_DEFECTINF * (rec - 0) + SIZE_DEFECTINF_HEADER + SIZE_DSP_TYP_DEFECTINF * (pnt - 1)
            m_typDefectInfPnt = CType(ksMemReadMap(TBL_HY_DEFECTINF_NAME, nReadAddr, tMod_MemSet.SizeOf(m_typDefectInfPnt), GetType(DEFECTINF_PNT), nRetc), DEFECTINF_PNT)
            If Not CBool(nRetc) Then
                ' 読み込み失敗
                DspShmemStatus(False)
                Exit Try
    ...
    Private Function ReadFromShmemRec(ByVal rec As Integer) As Boolean
        ...
        Try
            nTopByte = Marshal.SizeOf(GetType(DSP_MAP_DEFECTINF)) * rec
            m_typDspMapDefectInf = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nTopByte, GetType(DSP_MAP_DEFECTINF), ret), DSP_MAP_DEFECTINF)
    ...
    Private Function WriteToShmem(ByVal rec As Integer) As Boolean
        ...
        Try
            ...
            If Not ksMemWriteMap(TBL_HY_DEFECTINF_NAME, nTopByte, SIZE_DSP_MAP_DEFECTINF, m_typDspMapDefectInf) Then
                bRet = False
                DspShmemStatus(False)
            Else
                bRet = True
    ...
    Private Sub DefectInf1(ByVal rec As Integer, ByVal init As Boolean, ByVal flg As Boolean)
        ...
        With m_typDefectInfPnt.typ.data
                ...
                Try
                    Select Case ii
                        ...
                        Case 61 '疵発生時刻 秒
							...
							'// 2023.09.14 画像処理装置 H・Sソフト改造 -------------------->>>>>
						Case 62 'フレーム間連結フラグ
							If init Then dgv(0, ii).Value = "フレーム間連結フラグ" : dgv(1, ii).Value = "" _
							Else If flg Then dgv(1, ii).Value = .nIFMergeFlg.ToString _
							Else .nIFMergeFlg = CShort(dgv(1, ii).Value)
							'// 2023.09.14 画像処理装置 H・Sソフト改造 <<<<<--------------------
    ...
    Private Sub btnJmp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnJmp.Click
        ...
        Try
            ' 共有メモリ ヘッダ取得位置計算
            nTopByte = Marshal.SizeOf(GetType(DSP_MAP_DEFECTINF)) * rec
            typDefectInfHeader = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nTopByte, GetType(DEFECTINF_HEADER), nRetc), DEFECTINF_HEADER)
            If Not CBool(nRetc) Then
                ' 読み込み失敗
                DspShmemStatus(False)
                Exit Try
            ...
            For pnt = 1 To MAX_MAP_DEFECT_NUM
                ' 共有メモリ ポイント取得位置計算
                nTopByte = Marshal.SizeOf(GetType(DSP_MAP_DEFECTINF)) * rec + Marshal.SizeOf(GetType(DEFECTINF_HEADER)) + Marshal.SizeOf(GetType(DEFECTINF_PNT)) * (pnt - 1)
                typDefectInfPnt = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nTopByte, GetType(DEFECTINF_PNT), nRetc), DEFECTINF_PNT)
                If Not CBool(nRetc) Then
                    ' 読み込み失敗
                    DspShmemStatus(False)
                    Exit For
                ...
                ' 対象の疵連番のデータが見つかった
                If typDefectInfPnt.typ.data.nKizuNo = nKizuNo Then
                    bFound = True
                    Exit For
                End If
    ...
    Private Sub frmHyDefectInf_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        m_timer1.Stop()
        m_timer1.Dispose()
    End Sub
End Class