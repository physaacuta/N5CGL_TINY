...
// DummyFFDlg.cpp : 実装ファイル
//

#include "stdafx.h"
#include "DummyFF.h"
#include "DummyFFDlg.h"
...
CDummyFFDlg::CDummyFFDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_DUMMYFF_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CDummyFFDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST1, m_ListBox);
}

BEGIN_MESSAGE_MAP(CDummyFFDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDOK, &CDummyFFDlg::OnBnClickedOk)
	ON_LBN_DBLCLK(IDC_LIST1, &CDummyFFDlg::OnLbnDblclkList1)
	ON_WM_TIMER()
END_MESSAGE_MAP()


// CDummyFFDlg メッセージ ハンドラー

BOOL CDummyFFDlg::OnInitDialog()
{
...
//  システムがこの関数を呼び出します。
HCURSOR CDummyFFDlg::OnQueryDragIcon()
{
...
// 強制終了ボタン
//＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿
void CDummyFFDlg::OnBnClickedOk()
{
...
// メモ帳起動
//＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿
void CDummyFFDlg::OnLbnDblclkList1()
{
...
void CDummyFFDlg::InitProcess()
{
	...
	//	return;
	//}
	int nRetc = task_init(DummyFF);
	if( 0 != nRetc ) {
		if(-1 == nRetc) { syslog(SYSNO_TASK_INIT_ERR, "[%s] 既にタスク起動済みにより、起動中断。", DummyFF); }
		else			{ syslog(SYSNO_TASK_INIT_ERR, "[%s]", DummyFF); }
		_ASSERT(0);
		exit(-9);		// 自プロセス強制終了
		return;
	}
...
//------------------------------------------
unsigned WINAPI CDummyFFDlg::MainThread(void* param)
{
	CDummyFFDlg* p = (CDummyFFDlg *)param;
...
// 画面更新タイマ
//＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿
void CDummyFFDlg::OnTimer(UINT_PTR nIDEvent)
{
