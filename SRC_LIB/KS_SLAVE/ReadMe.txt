================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: KS_SLAVE プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した KS_SLAVE には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには KS_SLAVE アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
KS_SLAVE.h
   ...
   CKS_SLAVEApp アプリケーション クラスを宣言します。

KS_SLAVE.cpp
   これは、アプリケーション クラス CKS_SLAVEApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\KS_SLAVE.ico
   ...
   アイコンはメイン リソース ファイル KS_SLAVE.rc に含まれていま
   す。
...
KS_SLAVEDlg.h, KS_SLAVEDlg.cpp - ダイアログ
   これらのファイルは CKS_SLAVEDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な KS_SLAVE.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   KS_SLAVE.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
