================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: KS_SYSLOG プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した KS_SYSLOG には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには KS_SYSLOG アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
KS_SYSLOG.h
   ...
   CKS_SYSLOGApp アプリケーション クラスを宣言します。

KS_SYSLOG.cpp
   これは、アプリケーション クラス CKS_SYSLOGApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\KS_SYSLOG.ico
   ...
   アイコンはメイン リソース ファイル KS_SYSLOG.rc に含まれていま
   す。
...
KS_SYSLOGDlg.h, KS_SYSLOGDlg.cpp - ダイアログ
   これらのファイルは CKS_SYSLOGDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な KS_SYSLOG.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   KS_SYSLOG.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
