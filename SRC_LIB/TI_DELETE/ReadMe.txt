================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: TI_DELETE プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した TI_DELETE には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには TI_DELETE アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
TI_DELETE.h
   ...
   CTI_DELETEApp アプリケーション クラスを宣言します。

TI_DELETE.cpp
   これは、アプリケーション クラス CTI_DELETEApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\TI_DELETE.ico
   ...
   アイコンはメイン リソース ファイル TI_DELETE.rc に含まれていま
   す。
...
TI_DELETEDlg.h, TI_DELETEDlg.cpp - ダイアログ
   これらのファイルは CTI_DELETEDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な TI_DELETE.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   TI_DELETE.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
