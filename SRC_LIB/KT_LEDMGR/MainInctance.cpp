...
ThreadManager("MI")
{
	...
	// 初期化
	for(int ii=0; ii<MAX_THREAD; ii++) gcls_pLedMgr[ii] = NULL;
}
//------------------------------------------
// デストラクタ
//------------------------------------------
...
BOOL MainInctance::Init()
{
	...
	for(int ii=0; ii<MAX_THREAD; ii++) {
		...
		// インスタンス生成
		gcls_pLedMgr[ii] = new LedManager(cKey, ii, nBlock, nTempUnit, nTempThr, nTempThrDown, nTimerCycle);
		gcls_pLedMgr[ii]->SetLogMgr(mcls_pLog);
	}
	...
	for(int ii=0; ii<MAX_THREAD; ii++) {
		if(NULL == gcls_pLedMgr[ii]) break;
		gcls_pLedMgr[ii]->Start();
	}
...
void MainInctance::Exit()
{
	//// ワーカースレッド停止
	for(int ii=0; ii<MAX_THREAD; ii++) {
		if(NULL == gcls_pLedMgr[ii]) continue;
		gcls_pLedMgr[ii]->Stop(500);
	}

	//// ワーカースレッド解放
	for(int ii=0; ii<MAX_THREAD; ii++) {
		if(NULL == gcls_pLedMgr[ii]) continue;
		SAFE_DELETE( gcls_pLedMgr[ii] );
	}
}
...
//-----------------------------------------------------------
		case E_DEF_ML_PARAM:						// パラメータ変更通知
			if(true) {
				int scode = pMail->que.fl.data[0];
			
				// パラメータ変更コマンド
				for(int ii=0; ii<MAX_THREAD; ii++) {
					if(NULL==gcls_pLedMgr[ii]) continue;
					// パラメータ再取得
					gcls_pLedMgr[ii]->GetParam();

					// 現在の検査表面状態の場合、設定実施
					if(gcls_pLedMgr[ii]->GetNowScode() == scode || CH_SCODE_ALL == scode ) {
						gcls_pLedMgr[ii]->AddQue_SetLightMastVal(0,scode);
					}
				}
			}
			break;
