================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: KS_EVTLOG プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した KS_EVTLOG には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには KS_EVTLOG アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
KS_EVTLOG.h
   ...
   CKS_EVTLOGApp アプリケーション クラスを宣言します。

KS_EVTLOG.cpp
   これは、アプリケーション クラス CKS_EVTLOGApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\KS_EVTLOG.ico
   ...
   アイコンはメイン リソース ファイル KS_EVTLOG.rc に含まれていま
   す。
...
KS_EVTLOGDlg.h, KS_EVTLOGDlg.cpp - ダイアログ
   これらのファイルは CKS_EVTLOGDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な KS_EVTLOG.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   KS_EVTLOG.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
