...
class SyncopyManager : public WorkThread,
...
public:
	SyncopyManager(CString cSession, LogFileManager* pLog);
	virtual ~SyncopyManager(void);


	void Start() { WorkThread::Start(MainThread); };					// スレッド開始
	void Stop()  { WorkThread::Stop(10000); };							// スレッド終了
	
	char const*	GetType() const	{return (char const*)&"実績完全同期";};	// 動作タイプ (固定名称)

private :
	static UINT WINAPI MainThread(LPVOID param);						// メインスレッド
	bool Exec(bool bAtoB);												// A系の実績をB系に追加コピーさせる
