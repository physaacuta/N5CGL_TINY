...
class CopyManager : public WorkThread,
...
public:
	CopyManager(CString cSession, LogFileManager* pLog);
	virtual ~CopyManager(void);


	void Start() { WorkThread::Start(MainThread); };					// スレッド開始
	void Stop()  { WorkThread::Stop(10000); };							// スレッド終了
	
	char const*	GetType() const	{return (char const*)&"実績追加コピー";};	// 動作タイプ (固定名称)
