...
// コンストラクタ
//------------------------------------------
MoveManager::MoveManager(CString cSession, LogFileManager* pLog):
KizuSyncroManager(cSession, pLog)
{
	LOG(em_INF), "[MoveManager] [%s] 初期化開始 -------->>>>", my_cSession);
		...
		if( 0 < strlen(cWk) ) {
			...
			while ( strToken != "" ) {
				...
				if( 0 == rec ) {
					...
					if( PARAM_MAX_KEYCNT-1 <= m_Tbl[ii].nKeyCnt ) break;		// 主キーは２個までしか対応させていない為
					// 主キー指定
			...
			LOG(em_INF), "[MoveManager] T_%d ･･･ %s, [%d=%s,%s]", ii+1, m_Tbl[ii].cTbl, m_Tbl[ii].nKeyCnt, m_Tbl[ii].cKey[0], m_Tbl[ii].cKey[1]);
		}
	}
	LOG(em_INF), "[MoveManager] [%s] 初期化完了 <<<<--------", my_cSession);
}
...
// スレッド
//------------------------------------------
UINT WINAPI MoveManager::MainThread(LPVOID param)
{
	////// メインインスタンス取り出し
	MoveManager *p = (MoveManager *)param;				// 自クラスのインスタンスを取得
	p->SetTid();										// 自スレッドIDセット
	...
	// メッセージキュー読み込み
	pLOG(em_MSG), "[MoveManager] [%s] スレッド開始 [0x%X]", p->my_cSession, p->GetTid());
	while (1) {
...
//-----------------------------------------------------------------------------------------------
		case EV_STOP:												// スレッド終了
			pLOG(em_MSG), "[MoveManager] [%s] スレッド終了 [0x%X]", p->my_cSession, p->GetTid());
...
//-----------------------------------------------------------------------------------------------
		...
		case EV_EXEC_AtoB:											// A→B 処理開始
			pLOG(em_MSG), "[MoveManager] [%s] A→B 実績移動開始 -------------->>>", p->my_cSession);
		
			p->Exec(true);

			pLOG(em_MSG), "[MoveManager] [%s] A→B 実績移動完了 <<<--------------", p->my_cSession);
			break;


//-----------------------------------------------------------------------------------------------
		case EV_EXEC_BtoA:											// B→A 処理開始
			pLOG(em_WAR), "[MoveManager] [%s] B→A 実績移動開始 -------------->>>", p->my_cSession);
...
//------------------------------------------
bool MoveManager::Exec(bool bAtoB)
{
	//// メインとサブが生きている？
	if( ! CheckDB_Connect() ) {
		LOG(em_WAR), "[MoveManager] [%s] 2DB接続失敗", my_cSession);
		return false;
	}
	...
	for(int ii=0; ii<PARAM_MAX_TABLE; ii++) {
		if( m_Tbl[ii].bEnable ) {
			// 実績移動
			Move_DB(bAtoB, m_Tbl[ii].cTbl, m_Tbl[ii].nExecCnt, m_Tbl[ii].nKeyCnt, (char const*)m_Tbl[ii].cKey );	
