...
ThreadManager("MI")
{
	...
	// iniファイル読込み
	m_nIniThNum= GetPrivateProfileInt(KT_RPICHK, "INS_NUM", 0, TASKINI_NAME );

	// もろもろ初期化
	for(int ii=0; ii<MAX_THREAD; ii++) {
		gcls_pFileCheck[ii] = NULL;
	}
}
//------------------------------------------
// デストラクタ
...
BOOL MainInctance::Init()
{
	...
	for(int ii=0; ii<m_nIniThNum; ii++) {
		gcls_pFileCheck[ii] = new FileCheckManager(ii+1);
		gcls_pFileCheck[ii]->SetLogMgr(mcls_pLog);
	}
...
void MainInctance::Exit()
{
	//// ワーカースレッド 停止
	for(int ii=0; ii<m_nIniThNum; ii++) {
		gcls_pFileCheck[ii]->Stop();
	}

	//// ワーカースレッド 開放
	for(int ii=0; ii<m_nIniThNum; ii++) {
		delete gcls_pFileCheck[ii];
	}
}
