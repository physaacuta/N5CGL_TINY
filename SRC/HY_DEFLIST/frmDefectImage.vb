...
Public Class frmDefectImage
    ...
    Private m_typDefKey As DSP_LIST_DEFECT                          ' 疵情報(キー情報)
        ...
        Set(ByVal value As EM_MAP_SIDE)
            ...
            For Each uc As ucDefectImage In m_ucDefectImg
                If m_emSide = EM_MAP_SIDE.REVERSE Then           '左がDS
                    uc.lblImgSideR.Text = DIRECTION_NAME_DS
                    uc.lblImgSideL.Text = DIRECTION_NAME_WS
                Else                                            '左がWS
                    uc.lblImgSideL.Text = DIRECTION_NAME_DS
                    uc.lblImgSideR.Text = DIRECTION_NAME_WS
                End If
            Next
        End Set
    End Property
    ...
    ''' <remarks></remarks>
    Public Sub SetDefectData(ByVal key As DSP_LIST_DEFECT, ByVal data As DETECT_BASE_DATA, ByVal img() As Image, ByVal title() As String)
        ...
        Try
            ...
            For ii As Integer = 0 To m_ucDefectImg.Length - 1
                ...
                If title.Length > ii Then
                    m_ucDefectImg(ii).lblTitleName.Text = title(ii)
                End If
            Next
        Catch ex As Exception
            Log(LNO.ERR, String.Format("画像表示 メモリ設定異常[{0}]", ex.Message))
    ...
    ''' <remarks></remarks>
    Public Sub New(ByVal clsLog As tClass_LogManager, ByVal param As clsParamManager, ByVal strDefPath As String, ByVal strSection As String, ByVal nPicNum As UShort, ByVal bPicDirect As Boolean, Optional ByVal bPicRow As Boolean = False)
        ' この呼び出しはデザイナーで必要です。
        InitializeComponent()
        ...
        For ii As Integer = 0 To m_ucDefectImg.Length - 1
            m_ucDefectImg(ii) = New ucDefectImage(clsLog, strDefPath)
    ...
    Private Sub InitdgvDefInf(ByVal dgv As tCnt.tCnt_DataGridView)
        ...
        With dgv
            ...
            .DefaultCellStyle().Font = New System.Drawing.Font("ＭＳ 明朝", 8.5, System.Drawing.FontStyle.Bold, GraphicsUnit.Point)
            ...
            For ii As Integer = 0 To .Rows.Count - 1
                ...
                Select Case ii
                    ...
                    Case EM_ROW_DEFECTINF.WS
                        .Item(0, ii).Value = "WS～"
                    Case EM_ROW_DEFECTINF.SUCCESS
                        .Item(0, ii).Value = "周期性"
                    Case EM_ROW_DEFECTINF.SPEED
                        .Item(0, ii).Value = "速度"
                    Case EM_ROW_DEFECTINF.PITCH
                        .Item(0, ii).Value = "ﾋﾟｯﾁ"
                    Case EM_ROW_DEFECTINF.ROLL
                        .Item(0, ii).Value = "径"
        ...
        Try
            ...
            With dgvKizuInfo
                For ii As Integer = 0 To .RowCount - 1
                    Select Case ii
                        Case EM_ROW_DEFECTINF.PRODUCTNO
                            .Item(1, ii).Value = tMod_Common.ByteToString(m_typDefKey.cCoilNo).ToString
                        Case EM_ROW_DEFECTINF.NO
                        ...
                        Case EM_ROW_DEFECTINF.SUCCESS
                            If 0 <> m_DefectData.cycle_div Then
                                .Item(1, ii).Value = "有"
                                .Item(1, ii).Style.BackColor = g_ColorKizuCycle
                                .Item(1, ii).Style.ForeColor = Color.White
                            Else
                                .Item(1, ii).Value = "無"
                                .Item(1, ii).Style.BackColor = Color.White
                                .Item(1, ii).Style.ForeColor = Color.Black
                            End If
                        Case EM_ROW_DEFECTINF.PITCH
                            If 0 <> m_DefectData.cycle_div Then
