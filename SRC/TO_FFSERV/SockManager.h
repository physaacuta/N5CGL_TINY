...
private :
	...
	static const int PARAM_SOCKANSER_TIMEOUT= 1*60*1000;				// 応答伝文 タイムアウト(T1)
	...
	static const int PARAM_HELCYSEND_TIMER	= 1*60*1000;				// ヘルシー伝文 定周期送信 タイム(T3)
...
private:
	...
	void SetHelcySendTimer(int nTimer);									// ヘルシー伝文 最優先送信要求
	void SetJukosyoTimer();												// 重故障タイマーセット処理
	void FFAnserTimeOut();												// 応答伝文タイムアウト時の固有処理
	void AddQue_SendZiseki();											// 未送信実績の送信依頼
	...
	// 固有処理
	int SetRecvData( ExecItem* item, int nSize, BYTE const* data, char const* cTc ); // 受信データ処理
...
private:
	...
	// シグナル
	HANDLE					m_evAnserTimeOut;							// 応答伝文待ちタイムアウトシグナル
	HANDLE					m_evHelcySendTimer;							// ヘルシー伝文送信タイマーシグナル
	HANDLE					m_evJukosyoTimer;							// 重故障監視タイマーシグナル
