...
Public Class clsDispDefect
	...
	Private m_typDefInf As DSP_TYP_DEFECTINF = Nothing          '疵情報
	...
	''' <remarks></remarks>
	Public Sub New(ByVal typDefectDsp As TYP_DEFECT_DSP, ByRef param As clsParamManager, ByRef log As tClass_LogManager, ByRef point As clsPointInf, ByVal strPath As String)
		Try
...
#End Region
	...
	Private Sub DspDefInf()
		Try
			...
			'疵情報を表示する
			'-------------------------------------------------------------------
			Dim typData As DETECT_BASE_DATA = m_typDefInf.data
			...
			With m_typDefectDsp
				...
				If 1 = typData.nIFMergeFlg Then
					.lblType.BackColor = g_ColorTypeFMerge
				Else
					.lblType.BackColor = g_ColorControl
				End If
				'// 2023.09.14 画像処理装置 H・Sソフト改造 <<<<<--------------------
			...
			For Each dsp As tClass_ImageDsp2 In mcls_ImgDsp
				...
				Dim imgData As Image = GetImage(m_typDefInf, CType(nPos, EM_IMAGE_POS), m_typDefectDsp.lblImg(nPos).Text)
				m_typDefectDsp.lblCamNo.Text = m_typDefInf.data.camset.ToString ' カメラNo設定
				'---------------------------------------------------------------
				'疵画像を設定する
				'---------------------------------------------------------------
	...
	''' </summary>
	''' <remarks></remarks>
	Public Sub DspDefectRetry()

		Try
			' 無効レコードのチェック 疵が選択されていない場合などは -1をセットしている
			If 0 >= m_typMapValue.nRec Or m_typMapValue.nRec > TBL_HY_REC_MAX Then
			...
			For ii As Integer = 0 To strPath.Length - 1
				Select Case CType(mcls_Param.GetTypeImage(m_typDefInf.data.nTid, CType(ii, EM_IMAGE_POS)), EM_IMAGE_NO)
					Case EM_IMAGE_NO.S
						strPath(ii) = m_typDefInf.image_path_s
					Case EM_IMAGE_NO.R1
						strPath(ii) = m_typDefInf.image_path_r
					Case EM_IMAGE_NO.R2
						strPath(ii) = m_typDefInf.image_path_r2
					Case EM_IMAGE_NO.R3
						strPath(ii) = m_typDefInf.image_path_r3
					Case Else
						' 不定？
			...
			'疵情報ポインタ読み込み[共有メモリ]
			nReadAddr = SIZE_DSP_MAP_DEFECTINF * (m_typMapValue.nRec - 1) + SIZE_DEFECTINF_HEADER + SIZE_DSP_TYP_DEFECTINF * (m_typMapValue.nPnt - 1)
			m_typDefInf = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nReadAddr, GetType(DSP_TYP_DEFECTINF), nRetc), DSP_TYP_DEFECTINF)
			If Not CBool(nRetc) Then
				mcls_Log.Write(tClass_LogManager.LNO.ERR, String.Format("[{0}]疵情報再表示 疵情報ポインタ読み込み失敗[{1}]", TBL_HY_DEFECTINF_NAME, nReadAddr))
				Exit Try
			End If
			...
			If strPath(0) = "" Or strPath(1) = "" Then
				...
				For ii As Integer = 0 To strPathNew.Length - 1
					Select Case CType(mcls_Param.GetTypeImage(m_typDefInf.data.nTid, CType(ii, EM_IMAGE_POS)), EM_IMAGE_NO)
						Case EM_IMAGE_NO.S
							strPathNew(ii) = m_typDefInf.image_path_s
						Case EM_IMAGE_NO.R1
							strPathNew(ii) = m_typDefInf.image_path_r
						Case EM_IMAGE_NO.R2
							strPathNew(ii) = m_typDefInf.image_path_r2
						Case EM_IMAGE_NO.R3
							strPathNew(ii) = m_typDefInf.image_path_r3
						Case Else
							' 不定？
							strPathNew(ii) = String.Empty
					End Select
				Next
	...
	''' <param name="typMapValue">マップ情報</param>
	''' <param name="bReTry">リトライ有無</param>
	''' <remarks></remarks>
	Public Sub DspDefect(ByVal nMen As Integer, ByVal typMapValue As TYP_MAP_LIST_VALUE, Optional ByVal bReTry As Boolean = False)
		...
		Try
			'無効レコード
			If 0 >= typMapValue.nRec Or typMapValue.nRec > TBL_HY_REC_MAX Then
				mcls_Log.Write(tClass.tClass_LogManager.LNO.ERR, String.Format("[{0}]無効レコード[{1}]", TBL_HY_DEFECTINF_NAME, typMapValue.nRec))
				Exit Try
			End If

			'無効ポインタ
			If 0 >= typMapValue.nPnt Or typMapValue.nPnt > MAX_MAP_DEFECT_NUM Then
				mcls_Log.Write(tClass.tClass_LogManager.LNO.ERR, String.Format("[{0}]無効ポインタ[{1}]", TBL_HY_DEFECTINF_NAME, typMapValue.nPnt))
				Exit Try
			End If

			'メンバー変数
			m_typMapValue = typMapValue
			Dim nRec As Integer = m_typMapValue.nRec

			'疵情報ヘッダー読み込み[共有メモリ]
			'nReadAddr = SIZE_DSP_MAP_DEFECTINF * (nRec - 1)
			'm_typHeader = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nReadAddr, GetType(DEFECTINF_HEADER), nRetc), DEFECTINF_HEADER)
			'If Not CBool(nRetc) Then
			'    mcls_Log.Write(tClass.tClass_LogManager.LNO.ERR, String.Format("[{0}]疵情報ヘッダー読み込み失敗[{1}]", TBL_HY_DEFECTINF_NAME, nReadAddr))
			'    Exit Try
			'End If

			'疵情報ポインタ読み込み[共有メモリ]
			nReadAddr = SIZE_DSP_MAP_DEFECTINF * (nRec - 1) + SIZE_DEFECTINF_HEADER + SIZE_DSP_TYP_DEFECTINF * (typMapValue.nPnt - 1)
			m_typDefInf = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nReadAddr, GetType(DSP_TYP_DEFECTINF), nRetc), DSP_TYP_DEFECTINF)
			If Not CBool(nRetc) Then
				mcls_Log.Write(tClass_LogManager.LNO.ERR, String.Format("[{0}]疵情報ポインタ読み込み失敗[{1}]", TBL_HY_DEFECTINF_NAME, nReadAddr))
				Exit Try
			End If

			'画像パス無し
			If m_typDefInf.image_path_s Is Nothing Or m_typDefInf.image_path_s = "" Then
				If bReTry Then                              'リトライ有り
		...
		Catch ex As Exception
			mcls_Log.Write(tClass_LogManager.LNO.ERR, String.Format("nRec[{2}] pnt[{1}] nReadAddr[{3}] 疵情報表示異常[{0}]", ex.Message, typMapValue.nRec, typMapValue.nPnt, nReadAddr))
		End Try
	End Sub
	...
	Public Sub ShowPopup(ByVal clsPopup As frmDefectImage)
		...
		Try
			''無効レコード
			'If m_nNowRecord <= 0 Or m_nNowRecord > TBL_HY_REC_MAX Then
			'    mcls_Log.Write(tClass_LogManager.LNO.ERR, String.Format("[{0}]無効レコード[{1}]", TBL_HY_DEFECTINF_NAME, m_nNowRecord))
			'    Exit Try
			'End If

			''無効ポインタ
			'If m_typDspColRow.nPnt <= 0 Or m_typDspColRow.nPnt > MAX_MAP_DEFECT_NUM Then
			'    mcls_Log.Write(tClass.tClass_LogManager.LNO.ERR, String.Format("[{0}]無効ポインタ[{1}]", TBL_HY_DEFECTINF_NAME, m_typDspColRow.nPnt))
			'    Exit Try
			'End If

			''疵情報ヘッダー読み込み[共有メモリ]
			'nReadAddr = SIZE_DSP_MAP_DEFECTINF * (m_nNowRecord - 1)
			'm_typHeader = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nReadAddr, GetType(DEFECTINF_HEADER), nRetc), DEFECTINF_HEADER)
			'If Not CBool(nRetc) Then
			'    mcls_Log.Write(tClass_LogManager.LNO.ERR, String.Format("[{0}]疵情報ヘッダー読み込み失敗[{1}]", TBL_HY_DEFECTINF_NAME, nReadAddr))
			'    Exit Try
			'End If

			''疵情報ポインタ読み込み[共有メモリ]
			'nReadAddr = SIZE_DSP_MAP_DEFECTINF * (m_nNowRecord - 1) + SIZE_DEFECTINF_HEADER + SIZE_DEFECTINF_PNT * (m_typDspColRow.nPnt - 1)
			'typDefInf = CType(ksMemRead(TBL_HY_DEFECTINF_NAME, nReadAddr, GetType(DSP_TYP_DEFECTINF), nRetc), DSP_TYP_DEFECTINF)
			'If Not CBool(nRetc) Then
			'    mcls_Log.Write(tClass_LogManager.LNO.ERR, String.Format("[{0}]疵情報ポインタ読み込み失敗[{1}]", TBL_HY_DEFECTINF_NAME, nReadAddr))
			'    Exit Try
			'End If
			...
			For ii As Integer = 0 To clsImg.Length - 1
				clsImg(ii) = GetImage(m_typDefInf, CType(ii, EM_IMAGE_POS), tmpTitle(ii))
			Next
			...
			With clsPopup
				...
				''疵情報
				.SetDefectData(m_typDefInf.data, clsImg, tmpTitle)
				'表示倍率
				.ZoomRate = CInt((100.0 * m_typDefInf.data.w_res / m_nImageWidth) * 100.0)
				'目盛り描画詳細
