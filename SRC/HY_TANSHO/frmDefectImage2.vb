...
Public Class frmDefectImage2
    ...
    Private Enum EM_ROW_DEFECTINF
        ...
        NO                              ' 疵No
        DIV                             ' FF疵区分
        POS                             ' FF疵長さ方向位置 [MM]	※これは使わない
        IMAGEFLG                        ' 画像形式 (0:Jpeg 1:BMP)
        DSPMODE                         ' 事前編集フラグ (-1:非表示  0:通常表示  1:強調表示) ※プロビジョン
        MEN                             ' 表裏区分  (0:表 1:裏)
        CYCLE_DIV                       ' 周期疵区分(0:一般疵, 1:簡易周期疵,連続疵 2:周期欠陥)
        DSWS                            ' DSWS区分  (0:DS 1:WS)
        SPEED                           ' 速度[mpm]
        PITCH                           ' ピッチ[mm]
        ROOLKEI                         ' ロール径[mm]
        BOX_WID                         ' 疵幅 [mm]
        ...
        H_RES                           ' 縦分解能[mm/pixel]
        W_RES                           ' 横分解能[mm/pixel]
        BOX_X_MIN                       ' 外接Xmin[pixel]		(切り出し画像の左上からの位置) (マイナス値ありえる)
        BOX_X_MAX                       ' 外接Xmax[pixel]
        BOX_Y_MIN                       ' 外接Ymin[pixel]
        BOX_Y_MAX                       ' 外接Ymax[pixel]
        CENTER_X                        ' 疵中心位置X [pixel]  (切り出し画像の左上からの位置)
    ...
    Private mcls_Param As clsPreParamManager                        ' パラメータ管理
    ...
    Private m_DefectData As DSP_TYP_MAE_DEFECTINF                   ' 疵情報(前工程疵情報)
    Private m_ffinf As DSP_TYP_MAE_FFINF                            ' FF情報
    Private m_nKizuNo As Integer                                    ' 疵No
    ...
    ''' <remarks></remarks>
    Public Sub SetDefectData(ByVal nKizuNo As Integer, ByVal ffinf As DSP_TYP_MAE_FFINF, ByVal data As DSP_TYP_MAE_DEFECTINF, ByVal img() As Image, ByVal title() As String)
    ...
    ''' <param name="bPicRow">縦画像並び(false:無効、true：有効）</param>
    ''' <remarks></remarks>
    Public Sub New(ByVal clsLog As tClass_LogManager, ByVal param As clsPreParamManager, ByVal strDefPath As String, ByVal strSection As String, ByVal nPicNum As UShort, ByVal bPicDirect As Boolean, Optional ByVal bPicRow As Boolean = False)
        ' この呼び出しはデザイナーで必要です。
        InitializeComponent()

        ' InitializeComponent() 呼び出しの後で初期化を追加します。

        Me.Text = String.Format("[{0}] TCM疵拡大画像画面", SYSTEM_TITLE)
        ...
        Else
            ...
            For ii As Integer = 1 To nPicNum - 1
                ...
                tlpImage.RowCount = tlpImage.RowCount + 1
            Next
            ' 行幅を等分化
            Dim c As RowStyle
            For Each c In tlpImage.RowStyles
                c.SizeType = SizeType.Percent
                c.Height = CSng(100 / tlpImage.RowCount)
            Next
            nNum = tlpImage.RowCount
        End If
        ...
        For ii As Integer = 0 To m_ucDefectImg.Length - 1
            ...
            Else
                ' 縦並び(オプション)
                tlpImage.Controls.Add(m_ucDefectImg(ii), 0, pos)
            End If
    ...
    Public Sub SavePosition()
        tMod_IniFile.WriteIni(m_strSection, "TCM_IMAGE_POS_TOP", LOCALINI_NAME, CStr(Me.Top))
        tMod_IniFile.WriteIni(m_strSection, "TCM_IMAGE_POS_LEFT", LOCALINI_NAME, CStr(Me.Left))
        tMod_IniFile.WriteIni(m_strSection, "TCM_IMAGE_WINDOW_HEIGHT", LOCALINI_NAME, CStr(Me.Height))
        Dim nWidth As Integer = Me.Width
        If cbDetail.Checked Then
            nWidth = nWidth - dgvKizuInfo.Width
        End If
        tMod_IniFile.WriteIni(m_strSection, "TCM_IMAGE_WINDOW_WIDTH", LOCALINI_NAME, CStr(nWidth))
        tMod_IniFile.WriteIni(m_strSection, "TCM_IMAGE_DISP_RATE", LOCALINI_NAME, CStr(cmbPicRate.SelectedIndex))
    ...
    Public Sub ReadPosition()
        Dim nDispTop As Integer = tMod_IniFile.ReadIniInt(m_strSection, "TCM_IMAGE_POS_TOP", LOCALINI_NAME, -9999)
        If nDispTop >= Owner.Top And nDispTop <= Owner.Bottom Then
            Me.DispTop = nDispTop
        Else
            Me.DispTop = -9999
        End If
        Dim nDispLeft As Integer = tMod_IniFile.ReadIniInt(m_strSection, "TCM_IMAGE_POS_LEFT", LOCALINI_NAME, -9999)
        If nDispLeft >= Owner.Left And nDispLeft <= Owner.Right Then
            Me.DispLeft = nDispLeft
        Else
            Me.DispLeft = -9999
        End If
        Dim nSelIndex As Integer = tMod_IniFile.ReadIniInt(m_strSection, "TCM_IMAGE_DISP_RATE", LOCALINI_NAME, -1)
        If 0 <= nSelIndex Then
            cmbPicRate.SelectedIndex = nSelIndex
        End If

        Me.DispHeight = tMod_IniFile.ReadIniInt(m_strSection, "TCM_IMAGE_WINDOW_HEIGHT", LOCALINI_NAME, 640)
        Dim nWidth As Integer = tMod_IniFile.ReadIniInt(m_strSection, "TCM_IMAGE_WINDOW_WIDTH", LOCALINI_NAME, 720)
            ...
            For ii As Integer = 0 To .Rows.Count - 1
                ...
                Select Case ii
                    ...
                    Case EM_ROW_DEFECTINF.LENGTH
                        .Item(0, ii).Value = "欠陥長"
                    Case EM_ROW_DEFECTINF.PROC
                        .Item(0, ii).Value = "前工程"
                    Case EM_ROW_DEFECTINF.NO
                        .Item(0, ii).Value = "疵No."
                    Case EM_ROW_DEFECTINF.DIV
                        .Item(0, ii).Value = "疵区分"
                    Case EM_ROW_DEFECTINF.POS
                        .Item(0, ii).Value = "疵長方向"
                    Case EM_ROW_DEFECTINF.IMAGEFLG
                        .Item(0, ii).Value = "画像形式"
                    Case EM_ROW_DEFECTINF.DSPMODE
                        .Item(0, ii).Value = "事前編集"
                    Case EM_ROW_DEFECTINF.MEN
                    ...
                    Case EM_ROW_DEFECTINF.BOX_WID
                        .Item(0, ii).Value = "疵幅 "
                    Case EM_ROW_DEFECTINF.BOX_LEN
                        .Item(0, ii).Value = "疵長さ"
        ...
        Try
            ...
            With dgvKizuInfo
                For ii As Integer = 0 To .RowCount - 1
                    Select Case ii
                        Case EM_ROW_DEFECTINF.NO
                            .Item(1, ii).Value = m_nKizuNo.ToString
                        Case EM_ROW_DEFECTINF.TYPE
                            .Item(1, ii).Value = mcls_Param.GetDspTypeName(tMod.ByteToString(m_ffinf.cProc), tMod.ByteToString(m_ffinf.cT))
                        Case EM_ROW_DEFECTINF.GRADE
                            .Item(1, ii).Value = mcls_Param.GetDspGradeName(tMod.ByteToString(m_ffinf.cProc), tMod.ByteToString(m_ffinf.cG))
                            .Item(1, ii).Style.BackColor = tMod.GetIntToColor(mcls_Param.GetDspGradeColor(tMod.ByteToString(m_ffinf.cProc), tMod.ByteToString(m_ffinf.cG)))
                            .Item(1, ii).Style.SelectionBackColor = .Item(1, ii).Style.BackColor
                        Case EM_ROW_DEFECTINF.FPOS
                        	'// 2023.11.17 最終工程検査装置高機能化その４ -------------------->>>>>
                            '.Item(1, ii).Value = Format(Int(m_ffinf.nY / 100) / 10, "####0.0") 'F表示
                            .Item(1, ii).Value = Format(Int(m_ffinf.nY_Out / 100) / 10, "####0.0") 'F表示
							'// 2023.11.17 最終工程検査装置高機能化その４ <<<<<--------------------
                        Case EM_ROW_DEFECTINF.DS
                            .Item(1, ii).Value = m_ffinf.nWidth.ToString
                        Case EM_ROW_DEFECTINF.LENGTH
                            .Item(1, ii).Value = m_ffinf.nLen.ToString
                        Case EM_ROW_DEFECTINF.PROC
                            .Item(1, ii).Value = tMod.ByteToString(m_ffinf.cProc)
                        Case EM_ROW_DEFECTINF.DIV
                            .Item(1, ii).Value = m_ffinf.nDiv.ToString
                        Case EM_ROW_DEFECTINF.POS
                            .Item(1, ii).Value = m_ffinf.nPos.ToString
                        Case EM_ROW_DEFECTINF.IMAGEFLG
                            .Item(1, ii).Value = m_DefectData.nImageFlg.ToString
                        Case EM_ROW_DEFECTINF.DSPMODE
                            .Item(1, ii).Value = m_DefectData.nDspMode.ToString
                        Case EM_ROW_DEFECTINF.MEN
                            .Item(1, ii).Value = m_DefectData.men.ToString
                        Case EM_ROW_DEFECTINF.CYCLE_DIV
                        ...
                        Case EM_ROW_DEFECTINF.BOX_WID
                            .Item(1, ii).Value = m_DefectData.box_wid.ToString
                        Case EM_ROW_DEFECTINF.BOX_LEN
                            .Item(1, ii).Value = m_DefectData.box_len.ToString
                        Case EM_ROW_DEFECTINF.H_RES
            ...
            With dgvKizuInfo2
                For col As EM_COL_DEFECTINF = EM_COL_DEFECTINF.TYPE To EM_COL_DEFECTINF.MAX_COL
                    Select Case col
                        Case EM_COL_DEFECTINF.TYPE
                            .Item(col, 0).Value = mcls_Param.GetDspTypeName(tMod.ByteToString(m_ffinf.cProc), tMod.ByteToString(m_ffinf.cT))
                        Case EM_COL_DEFECTINF.GRADE
                            .Item(col, 0).Value = mcls_Param.GetDspGradeName(tMod.ByteToString(m_ffinf.cProc), tMod.ByteToString(m_ffinf.cG))
                            .Item(col, 0).Style.BackColor = tMod.GetIntToColor(mcls_Param.GetDspGradeColor(tMod.ByteToString(m_ffinf.cProc), tMod.ByteToString(m_ffinf.cG)))
                            .Item(col, 0).Style.SelectionBackColor = .Item(col, 0).Style.BackColor
                        Case EM_COL_DEFECTINF.FPOS
                            ' 品種区分
                            '// 2023.11.17 最終工程検査装置高機能化その４ -------------------->>>>>
                            '.Item(col, 0).Value = Format(Int(m_ffinf.nY / 100) / 10, "####0.0") 'F表示
							.Item(col, 0).Value = Format(Int(m_ffinf.nY_Out / 100) / 10, "####0.0") 'F表示
							'// 2023.11.17 最終工程検査装置高機能化その４ <<<<<--------------------
                    End Select
                Next
            End With
