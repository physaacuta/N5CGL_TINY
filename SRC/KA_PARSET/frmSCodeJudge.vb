...
Public Class frmSCodeJudge
    ...
    ''' <remarks></remarks>
    Private Sub frmSCodeJudge_tEv_FunctionButton(ByVal FncNo As Integer, ByVal enable As Boolean, ByVal onKey As System.Windows.Forms.Keys) Handles Me.tEv_FunctionButton
        '' ファンクションボタン有効項目のみ
        ...
        Select Case FncNo                       ' ※0オリジン
            ...
            Case 6                              ' F07 画面印刷
                Call PrintOut()
            Case 7                              ' F08 画面保存
                Call HardCopy(IMG_PATH, "検査表面状態条件設定画面_" & Format(Now, "yyyyMMdd_HHmmss") & ".jpg", Drawing.Imaging.ImageFormat.Jpeg)
            Case 8
    ...
    ''' <remarks></remarks>
    Private Sub frmSCodeJudge_tEv_SelectMenuList(ByVal frmID As Integer, ByRef cancel As Boolean) Handles Me.tEv_SelectMenuList
        ' いろいろ開放処理
        m_DS.Dispose()
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub frmSCodeJudge_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        modMain.SetListMenu(Me)                                     ' メニューリスト生成
        modMain.SetLabelSysName(Me)                                 ' システム名ラベル生成
        MyBase.tProp_MyFrmID = EM_FRM_ID.emfrmSCodeJudge            ' 検査表面状態条件設定画面呼出
        ...
        ' コンボボックスの"デフォルト値(MAX_SCODE)を削除する
        MyBase.Delete_CodeEtc(cmbUpCodeItem, CStr(MAX_SCODE))
        ...
        ' コンボボックスのデフォルト値(MAX_SCODE)を削除する
        MyBase.Delete_CodeEtc(cmbSCode, CStr(MAX_SCODE))
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frmSCodeJudge_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
    ...
    Private Function DB_Select() As Boolean
        ...
        strSQL &= " FROM " & DB_SCODE_JUDGE
        strSQL &= " ORDER BY scode ASC, 条件No ASC"

        ' SQL文実行
        If 0 > MyBase.Get_SelectData(strSQL, m_DS, m_Adapter, strErrMsg) Then
            gcls_Log.Write(tClass_LogManager.LNO.ERR, DB_SCODE_JUDGE & " 読込エラー [ " & strErrMsg & "] " & Me.Text)
            Return False
        End If
    ...
    Private Sub Data_Disp(ByVal key As String)
        ...
        For ii As Integer = 0 To drow.Length - 1
            ...
            If drow(ii).Item("SPM圧下有無").ToString.Trim = "0" Then
                dgvData(EM_DATACOLUMN.emCmb_Spm, ii).Value = "×:圧下無"
            ElseIf drow(ii).Item("SPM圧下有無").ToString.Trim = "1" Then
                dgvData(EM_DATACOLUMN.emCmb_Spm, ii).Value = "○:圧下有"
            Else
                dgvData(EM_DATACOLUMN.emCmb_Spm, ii).Value = "無し"
            End If
    ...
    Private Sub Data_Get(ByVal key As String)
        ...
        ' 条件No分ループ
        For ii As Integer = 0 To dgvData.Rows.Count - 1
            strWhere = String.Format("scode='{0}' and 条件No={1}", key, ii + 1)     ' WHERE句に相当の条件生成
            ...
            If (dgvData(EM_DATACOLUMN.emTxt_MeireiNo, ii).Value Is System.DBNull.Value Or
               ...
               (dgvData(EM_DATACOLUMN.emTxt_HinSyu, ii).Value Is System.DBNull.Value Or
                ...
                dgvData(EM_DATACOLUMN.emCmb_Spm, ii).Value.ToString.Trim = "無し" AndAlso
            ...
            If dgvData(EM_DATACOLUMN.emCmb_Spm, ii).Value.ToString.Trim = "×:圧下無" Then
                drow.Item("SPM圧下有無") = 0
            ElseIf dgvData(EM_DATACOLUMN.emCmb_Spm, ii).Value.ToString.Trim = "○:圧下有" Then
                drow.Item("SPM圧下有無") = 1
            Else
                drow.Item("SPM圧下有無") = System.DBNull.Value
            End If
    ...
    Private Function DB_UpDate() As Boolean
        ...
        Try
            ...
            ' 統括機能へパラメータ変更通知
            modMain.Send_Main(CH_PARAM_SETTIG_SCODE)
            modMain.HardCopy(Me)
            Return True
        Catch ex As Exception
    ...
    Private Overloads Sub dgvInitialize(ByVal dgv As tClassLibrary.tCnt.tCnt_DataGridView)
        ...
        ' 条件No
        dgvColSetText(dgv, "条件NO", 60, True, True,
        ...
        Using clm As New DataGridViewComboBoxColumn
            clm.HeaderText = "SPM圧下有無"
            clm.Width = 150
            clm.SortMode = DataGridViewColumnSortMode.NotSortable
            clm.Frozen = False
            clm.DefaultCellStyle.Alignment = DataGridViewContentAlignment.TopCenter
            ...
            lstItem.Items.AddRange("無し", "○:圧下有", "×:圧下無")
    ...
    Private Function CopyDB(ByVal strFromKey As String, ByVal strToKey As String) As Boolean
        ...
        '''' SQL文の生成
        strSQL = "UPDATE " & DB_SCODE_JUDGE & " SET "
        ' 可変列
        strSQL &= tClass_SQLServer.sqlColumnUpdate("有効判定", "b")
        strSQL &= tClass_SQLServer.sqlColumnUpdate("SPM圧下有無", "b")
        ...
        ' From句
        strSQL &= "FROM " & DB_SCODE_JUDGE & " a "
        strSQL &= ", (SELECT * FROM " & DB_SCODE_JUDGE
        strSQL &= " WHERE scode='" & strFromKey & "') b "

        ' WHERE句
        strSQL &= "WHERE a.条件No=b.条件No AND a.scode IN(" & strToKey & ")"

        ' SQL文実行
        If 0 > MyBase.UpCode_UpDate(strSQL, strErrMsg) Then
            gcls_Log.Write(tClass_LogManager.LNO.ERR, DB_SCODE_JUDGE & " 更新エラー [" & strErrMsg & "] " & Me.Text)
            Return False
        End If
